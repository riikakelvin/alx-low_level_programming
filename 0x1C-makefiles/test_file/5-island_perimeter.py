...
def island_perimeter(grid):
    ...
    Raises:
        ValueError: If the grid is not rectangular, or if the island has lakes 
    ...
    for k in range(rows):
        for e in range(cols):
            if grid[k][e] == 1:
                # check left
                if e == 0 or grid[k][e-1] == 0:
                    perimeter += 1
                # check right
                if e == cols-1 or grid[k][e+1] == 0:
                    perimeter += 1
                # check top
                if k == 0 or grid[k-1][e] == 0:
                    perimeter += 1
                # check bottom
                if k == rows-1 or grid[k+1][e] == 0:
                    perimeter += 1
